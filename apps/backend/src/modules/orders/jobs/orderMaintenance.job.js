...
export const initOrderMaintenanceJob = () => {
  ...
  const job = cron.schedule('*/5 * * * *', async () => {
    // 4 minutes 30 seconds lock duration to prevent overlapping
    const lockDurationMs = 270000; 

    await jobManager.executeWithLock('orderMaintenance', lockDurationMs, async () => {
